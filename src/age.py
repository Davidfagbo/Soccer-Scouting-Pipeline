"""Reconcile StatsBomb player names against birthdates and filter to an age range.
...
def _search_wikidata(name: str) -> str | None:
    """Return the QID of the best football-player match for `name`, or None."""
...
def _best_birthdate_claim(claims: list[dict]) -> dict | None:
    """Pick the claim Wikidata itself prefers: a 'preferred'-rank claim if one
    exists, otherwise the first non-deprecated claim. Some contested-birthdate
    players carry more than one P569 statement.
    """
...
def _fetch_birthdates(qids: list[str]) -> dict[str, date]:
    """Batch-fetch P569 (date of birth) for up to 50 QIDs per call.

    Only day-precision claims (Wikidata precision=11) are accepted: a
    year- or month-only birthdate can't compute an accurate age, and
    silently defaulting the missing day to the 1st could misclassify a
    player's age band by up to ~11 months. Anything less precise is left
...
def resolve_birthdates(
    ...
    """Resolve StatsBomb player names to birthdates.

    Manual overrides (if present) always win, then the local cache, then a
    live Wikidata lookup. Returns columns [player_name, birth_date, source].
    Unresolved names are written to `unmatched_log_path`, not dropped.
    """
...
def attach_age(players: pd.DataFrame, birthdates: pd.DataFrame, reference_date: date) -> pd.DataFrame:
    """Join `birthdates` onto `players` (on player_name) and compute age as of
    `reference_date`. Rows with no resolved birthdate keep age=NaN, so callers
    can distinguish "too old/young" from "unresolved" rather than dropping
    either silently.
    """
